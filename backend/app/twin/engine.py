...
class DigitalTwinEngine:
    ...
    def load_topology(self, config_path: str) -> None:
        ...
        {
          "nodes": [
            {"id": "Web-01", "type": "server", "name": "Web Server 01",
             "criticality": "high", "zone": "dmz"}
          ],
          "edges": [
            {"source": "Web-01", "target": "DB Primary",
             "type": "network", "protocols": ["tcp"], "weight": 1.0,
             "bidirectional": false}
          ]
        }

        Args:
            config_path: Path to the JSON topology file
        """
        path = Path(config_path)
        if not path.is_file():
            logger.error("Topology file not found", path=config_path)
            raise FileNotFoundError(f"Topology file not found: {config_path}")
    ...
    def simulate_attack(self, compromised_node: str) -> List[List[str]]:
        """
        Simulate an attack starting from a compromised node.

        Returns all reachable paths from the compromised node to every
        other node using networkx shortest_path.

        Args:
            compromised_node: The initially compromised node ID

        Returns:
            List of paths, each path is a list of node IDs, e.g.
            [["Web-01","DB Primary"], ["Web-01","LoadBalancer","Firewall"]]
        """
        if compromised_node not in self.graph:
            logger.warning("Node not in graph", node=compromised_node)
            return []
        ...
        for target in self.graph.nodes():
            if target == compromised_node:
                continue
            try:
                path = nx.shortest_path(self.graph, compromised_node, target)
                paths.append(path)
