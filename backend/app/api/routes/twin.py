...
async def simulate_attack(
    ...
    """
    Simulate an attack from a compromised node.

    Loads the network topology and returns all reachable
    shortest paths from the compromised node.
    """
    twin = get_twin_engine()

    # Load topology from file if graph is empty or node not found
    if twin.graph.number_of_nodes() == 0 or request.compromised_node not in twin.graph:
        import os
        # routes/twin.py -> routes -> api -> app -> backend
        backend_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        topo_path = os.path.join(backend_dir, "data", "network_topology.json")
        if os.path.isfile(topo_path):
            twin.load_topology(topo_path)
        elif twin.graph.number_of_nodes() == 0:
            twin.initialize_sample_network()

    if request.compromised_node not in twin.graph:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Node '{request.compromised_node}' not found in topology",
        )

    paths = twin.simulate_attack(request.compromised_node)

    return {
        "compromised_node": request.compromised_node,
