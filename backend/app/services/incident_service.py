...
class IncidentService:
    ...
    async def create_from_risk(
        ...
        # Run Digital Twin attack simulation if asset is available
        asset_name = None
        if risk.asset:
            asset_name = risk.asset.hostname or risk.asset.name
        if asset_name:
            try:
                twin = get_twin_engine()
                if not twin.graph.nodes:
                    from pathlib import Path
                    topo_path = str(Path(__file__).resolve().parent.parent.parent / "data" / "network_topology.json")
                    twin.load_topology(topo_path)
                attack_paths = twin.simulate_attack(asset_name)
                await self.save_analysis(
                    incident_id=incident.id,
                    agent_type="digital_twin",
                    analysis_result={
                        "compromised_node": asset_name,
                        "reachable_targets": len(attack_paths),
                        "attack_paths": attack_paths,
                    },
                    confidence_score=85.0,
                    recommendations=[
                        f"Isolate {asset_name} to prevent lateral movement",
                        f"{len(attack_paths)} reachable targets identified — review network segmentation",
                    ],
                )
                logger.info("Digital Twin simulation attached", incident_id=str(incident.id), asset=asset_name, paths=len(attack_paths))
            except Exception as e:
                logger.error("Digital Twin simulation failed", error=str(e))
