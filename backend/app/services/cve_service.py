...
class CVEService:
    ...
    async def search_by_product(
        ...
        Args:
            ...
            product: Alias for product_name (route compat)
        ...
        """
        search_product = product_name or product or ""
        
        # affected_software stores strings like "vendor:product"
        search_term = f"{vendor}:{search_product}" if vendor else search_product
        
        query = select(CVE).where(
            CVE.affected_software.cast(String).ilike(f"%{search_term}%")
        )
