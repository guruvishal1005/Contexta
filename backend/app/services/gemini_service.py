...
RULES:
1. Each agent speaks EXACTLY ONCE — do NOT repeat any agent
2. Each agent MUST reference findings from previous agents in their stage
3. Messages should show progressive understanding and escalation
4. Use realistic SOC terminology and be specific to the risk title
5. Return ONLY valid JSON with exactly {len(selected_agents)} messages (one per agent)
6. timestamp_offset_seconds should increase progressively (0, 15, 45, 90, 120)
7. Each agent's single message should be comprehensive — combine all their analysis into one detailed response
