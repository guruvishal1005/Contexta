...
def _add_default_jobs(scheduler: AsyncIOScheduler) -> None:
    ...
    # CVE collection job - uses configured interval
    scheduler.add_job(
        collect_cves_job,
        trigger=IntervalTrigger(hours=settings.cve_fetch_interval_hours),
        id="collect_cves",
...
async def collect_cves_job() -> None:
    ...
    try:
        ...
        # Collect from both CISA KEV and NVD, merged and deduplicated
        all_cves = await collector.collect_all(days_back=7)
        await collector.close()
        
        logger.info("CVE collection complete", total_fetched=len(all_cves))
        ...
        logger.info("CVE persistence complete", stored=stored)
